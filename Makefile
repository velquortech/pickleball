...
setup-supabase: init-log
	...
	@if [ -s supabase_output.txt ]; then \
		...
		echo "PAYMENTS_PROVIDER=mock" >> .env; \
...
seed:
	@yarn supabase db reset
	@echo "Database reset — migrations applied and seeders executed"

generate-types:
	yarn --silent supabase gen types typescript --local > config/supabase/types.ts
	@echo "Types generated at config/supabase/types.ts"
...
.PHONY: all init-log setup-supabase clean migrate-new migrate-up migrate-diff migrate-reset seed generate-types stop-db run-dev run-start start-app build-app test test-unit test-integration test-watch
